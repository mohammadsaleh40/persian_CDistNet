...
Result process_task(const Task& task) {
    ...
    // رندر کلمه با تغییر مکان تصادفی - استفاده از تابع جدید
    draw_text_with_random_placement(task.word, task.font_path, result.image, font_size, gen);
...
int main() {
    ...
    std::string word_list_path = "littel.txt";
    ...
    fs::create_directory("dataset/little");
    std::string image_folder = "dataset/little";
    fs::create_directory(image_folder);

    // باز کردن فایل annotation
    std::ofstream annotation_file("dataset/little/gt.txt");
    if (!annotation_file.is_open()) {
        std::cerr << "Could not create annotation file dataset/little/gt.txt" << std::endl;
        return -1;
    }
        ...
        for (int j = 0; j < batch_size; ++j) {
            ...
            if (result.success) {
                ...
                if (write_result == 0) {
                    ...
                    annotation_file << result.filename << ", \"" << result.word << "\"" << std::endl;
                    total_images++;
                }
            }
            ...
            if (completed % 100 == 0 || completed == tasks.size()) {
