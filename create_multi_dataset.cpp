...
int main() {
    ...
    fs::create_directory("dataset/big");
    std::string image_folder = "dataset/big";
    fs::create_directory(image_folder);

    // باز کردن فایل annotation
    std::ofstream annotation_file("dataset/big/gt.txt");
    if (!annotation_file.is_open()) {
        std::cerr << "Could not create annotation file dataset/big/gt.txt" << std::endl;
        return -1;
    }
